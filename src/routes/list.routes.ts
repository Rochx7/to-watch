...
export async function listRoutes(fastify: FastifyInstance){
  ...
  fastify.get<{Params: {id:number}}>("/:id", async(req, reply)=>{
    const { id } = req.params
    try{
      const data = await listUseCase.getList({idUser: Number(id)})
  ...
  fastify.put<{Body:{itemToAdd:string, idUser:number}}>("/", async(req, reply)=>{

    const {idUser, itemToAdd} = req.body

    try{
      const data = {idUser,itemToAdd}
      const response = await listUseCase.addNewInWatchList(data)
